...
{  int err;
  ...
  */         
  ...
  // Assign parameters for F3SuR model 
  double MXs, MYF3u1, lamF3u, delta_small;
  MXs = 1000.; // DM mass in GeV
  delta_small = 5.e-3; // Mass splitting between DM and mediator
  MYF3u1 = MXs*(1. + delta_small); // Mediator mass in GeV
  lamF3u = 0.01; // t-channel Yukawa coupling
  assignValW("lamF3u", lamF3u); // Yukawa coupling
	assignValW("MXs", MXs); // DM mass in GeV
	assignValW("MYF3u1", MYF3u1); // Mediator mass in GeV
