...
/* If our SE+BSF4DM is used, these two files are included in 
"lib/improveCrossSection_Sommerfeld.cpp" and "lib/BoundStateFormation.cpp" directly
#include"../include/micromegas.h"
#include"../include/micromegas_aux.h" 
*/
#include"lib/pmodel.h"
#include"lib/improveCrossSection_Sommerfeld.cpp"
#include"lib/BoundStateFormation.cpp"
...
{  int err;
  ...
  */  
  ...
  // Assign parameters for S3Muni model 
  double MXm, MYS3u1, MYS3u2, MYS3u3, lamS3u, delta_small;
  MXm = 1000.; // DM mass in GeV
  delta_small = 5.e-3; // Mass splitting between DM and mediator
  MYS3u1 = MXm*(1. + delta_small); // Mediator mass to up in GeV
  MYS3u2 = MXm*(1. + delta_small); // Mediator mass to charm in GeV
  MYS3u3 = MXm*(1. + delta_small); // Mediator mass to top in GeV
  lamS3u = 0.0001; // t-channel Yukawa coupling (taken to be flavor universal here)

  assignValW("lamS3u1x1", lamS3u); // Yukawa coupling to up
  assignValW("lamS3u2x2", lamS3u); // Yukawa coupling to charm
  assignValW("lamS3u3x3", lamS3u); // Yukawa coupling to top
	assignValW("MXm", MXm); // DM mass in GeV
	assignValW("MYS3u1", MYS3u1); // Mediator mass in GeV
	assignValW("MYS3u2", MYS3u2); // Mediator mass in GeV
	assignValW("MYS3u3", MYS3u3); // Mediator mass in GeV
...
{ int fast=1;
  ...
  {  double Xf;
     ...
     Omega = darkOmegaExt(&Xf, BSF_XS_A, BSF_XS_S);
